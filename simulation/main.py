...
def run(betas):
    ...
    main = Main()
    main.initialize_inputs()
    # main.inputs.policy_name = 'itinerary'
    main.inputs.vehicles_file_name = f'../../input_data_generator/output/instance{instance}/vehicles.csv' 
    main.inputs.demand_file_name = f'../../input_data_generator/output/instance{instance}/original_demand.csv'  
    main.inputs.itineraries_file_name = f'../../input_data_generator/output/instance{instance}/itineraries.csv' 
    main.run_simulation(betas = betas)
    # main.run_simulation(betas = numpy.zeros((5,11,5,11)))
    
    # output_list = []
    # for t in range(len(main.policy_connector.pre_decision_states_values)):
    #     value = main.policy_connector.pre_decision_states_values[t]
    #     state = list(main.policy_connector.pre_decision_states[t].flatten())
    #     output_list.append(tuple([value] + state))
    # df = pandas.DataFrame(output_list, columns = None)
    # df.to_csv(f'../../input_data_generator/output/instance{instance}/pre_decision_state_values.csv', index = False, header = False)
...
if __name__ == '__main__':
    df = pandas.read_csv('betas.csv', header = None, index_col = None)
    betas = list(df[0])
    betas = numpy.array(betas)
    betas = betas.reshape((5,11,5,11))
    run(betas)
    #root_function()
    # run()
    # t0 = time.time(); 
